import {Injectable} from '@angular/core';
import 'rxjs/add/operator/map';
import {Factorys} from '../../theme/factorys';
import {HttpService} from  '../http-service';

@Injectable()
export class HandleOrderService {

  constructor(public httpService: HttpService, public factorys: Factorys) {
  }

  post({orderNo, type, code, desc, machineId}) {
    return new Promise((resolve, reject) => {
      this.factorys.toJsonString({orderNo, type, code, desc, machineId}).then((json)=> {
        this.httpService.operativeTaskOrderHandleOrder(json).map((res) => res.json()).subscribe((data) => {
          this.factorys.checkHttpData(data) && resolve(data);
        }, (err) => {
          this.factorys.httpError();
        });
      });
    });
  }
}
