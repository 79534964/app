...
@Injectable()
export class ProductIdListService {

  constructor(public httpService: HttpService, public factorys: Factorys) {
  }

  post({productId, startDate, endDate, pageNumber = 1, pageSize = 365}) {
    return new Promise((resolve, reject) => {
      this.factorys.toJsonString({
        ...
        this.httpService.countProductIdList(json).map((res) => res.json()).subscribe((data) => {
          if (this.factorys.checkHttpData(data)) {
            ...
            chartConfig.title = data['content'].list[0].productName;
