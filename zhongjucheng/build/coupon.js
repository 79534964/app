...
/***/ function(module, exports) {
	...
	exports.setData=function(e,flag){
	    if(flag){
	        var imgSrc="http://zjc.mattburg.cn/jeewxmb/webpage/extend/images/";    
	    }else{
	        var imgSrc="http://zjc.mattburg.cn/jeewxmb/webpage/extend/images/n";
	    }
	    var imgName;
	    var name;
	    var html;
	    if(e.priceType==1){
	        name=e.ruleName;
	        if(e.couponPrice>5||e.couponPrice*1==1){
	            imgName=99;
	        }else{
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports=function(data){
	    ...
	    $.each(arr,function(i,e){
	        ...
	        if(num<=0){
	            nodes.eq(i).append("<img class='rob y no-break' index='"+i+"' src='http://zjc.mattburg.cn/jeewxmb/webpage/extend/images/coupon-y.png' alt=''/>");
	        }else if(num<=(1000*60*60*24)){
	            nodes.eq(i).append("<img class='rob n break' index='"+i+"' src='http://zjc.mattburg.cn/jeewxmb/webpage/extend/images/coupon-n.png' alt=''/><span></span>");
	            newArr.push(num);
	        }else{
	    ...
	    function countdown(arr){
	        ...
	        for(var i=0;i<arr.length;i++){
	            if(arr[i]<1000){
	                nodes.eq(i).attr({"src":"http://zjc.mattburg.cn/jeewxmb/webpage/extend/images/coupon-y.png","class":"rob y no-break"}).next().html("");
	                arr.splice(i,1);
	            }else{
	                arr[i]=arr[i]-1000;
	...
	module.exports=function(nodes,couponOffList){
	    nodes.on('click',function(){
	        ...
	        if(node.attr("class").indexOf("y")!=-1){
	            ...
	            var url="http://zjc.mattburg.cn/jeewxmb/productController.do?getCouponOff&"+'openId='+openId+'&coupon_rule_id='+coupon_rule_id;
	            $.ajax({
	                  type: 'GET',
	                  url: url,
	                  dataType: "json",
	                  success: function(data){
	                    ...
	                    if(data[0].msg.indexOf("成功")!=-1){
	                        window.location.href="http://zjc.mattburg.cn/jeewxmb/productController.do?toCoupon";
	                    }
	                  }
	            });
	        }else{
